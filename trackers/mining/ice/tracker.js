...
function initialFetch() {
    ...
    .then(data => {
        ...
        console.log(data)
        collectionStart = data.profile.members[UUIDNoDashes].collection.ICE
...
function updateStats() {
    ...
    .then(data => {
        console.log("updated")
        lastUpdateCheck = 30
        totalCollection = data.profile.members[UUIDNoDashes].collection.ICE
...
function getUpdatedSellPrice() {
  ...
  .then(data => {
    console.log(data)
    console.log(data.products.ICE.sell_summary[0].pricePerUnit)
