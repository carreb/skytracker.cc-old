...
function initialFetch() {
    ...
    .then(data => {
        ...
        console.log(data)
        collectionStart = data.profile.members[UUIDNoDashes].collection.HARD_STONE
...
function updateStats() {
    ...
    .then(data => {
        console.log("updated")
        lastUpdateCheck = 30
        totalCollection = data.profile.members[UUIDNoDashes].collection.HARD_STONE
...
function getUpdatedSellPrice() {
  ...
  .then(data => {
    console.log(data)
    console.log(data.products.HARD_STONE.sell_summary[0].pricePerUnit)
