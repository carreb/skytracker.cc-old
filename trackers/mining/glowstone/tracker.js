...
function initialFetch() {
    ...
    .then(data => {
        ...
        console.log(data)
        collectionStart = data.profile.members[UUIDNoDashes].collection.GLOWSTONE_DUST
...
function updateStats() {
    ...
    .then(data => {
        console.log("updated")
        lastUpdateCheck = 30
        totalCollection = data.profile.members[UUIDNoDashes].collection.GLOWSTONE_DUST
...
function getUpdatedSellPrice() {
  ...
  .then(data => {
    console.log(data)
    console.log(data.products.GLOWSTONE_DUST.sell_summary[0].pricePerUnit)
